...
from myapp.forms import UserForm,RoomForm
from myapp.models import User,room,room_members,transaction,debt,final_transactions,Personal_income,Personal_expense
...
def joinus(request):
    return render(request,'../templates/joinus.html',{})
...
def add_room(request):
    if request.method == "POST":
        ...
        member.save()
        return HttpResponseRedirect(reverse('myapp:room_list'))
    return render(request,'../templates/room_list.html',{'form':form})
...
def room_list(request):
    ...
    return render(request,'../templates/room_list.html',{'rooms_list':rooms_list,'length':length})
...
def room_details(request,pk):
    ...
    return render(request,'../templates/room_detail.html',{'rooms':rooms,
...
def add_member(request,pk,id):
    ...
    room_member.save()
    return HttpResponseRedirect(reverse('myapp:list_members',kwargs={'pk':pk}))
...
def list_members(request,pk):
    ...
    template = "../templates/members_list.html"
...
amount,reason,myapps and all details of transaction.
"""
def transaction_details(request,pk):
    ...
    transaction_myapps = transactions.myapps.all()
    transaction_myapps_username = [x.username for x in transaction_myapps]
    return render(request,'../templates/transaction_details.html',{'transaction':transactions,'transaction_myapps_username':transaction_myapps_username,'all_room_members':all_room_members,'transaction_myapps':transaction_myapps})

# Create transaction View
# Take all the data of transaction and create transaction object. Payer will be the current user.
# Take the list of myapps and make debt object for each of them.
# For all the myapps, filter the objects in final_transactions model with user and another person.
...
def create_transaction(request,pk):
    ...
    if request.method == "POST":
        try:
            ...
            transaction_members = request.POST.getlist('myapp')
            myapps = User.objects.filter(username__in = transaction_members)
            obj = transaction(room = rooms,amount=amount,reason=reason,payer=request.user)
            obj.save()
            obj.myapps.add(*myapps)
            for x in myapps:
                if x == request.user:
                    pass
                else:
                    debt_obj = debt(room = rooms,transaction=obj,sender=x,receiver=request.user,amount=int(amount)/len(myapps))
                    debt_obj.save()
                    ...
                    if len(if_user_receiver) == 0 and len(if_user_sender) == 0:
                        final_obj = final_transactions(sender=x,receiver=request.user,final_amount=int(amount)/len(myapps))
                        final_obj.save()
                    else:
                        if len(if_user_receiver) == 1 and len(if_user_sender) == 0:
                            ...
                            final_objs.final_amount += int(amount)/len(myapps)
                            final_objs.save()
                        else:
                            final_objs = final_transactions.objects.get(sender=request.user,receiver=x)
                            final_objs.final_amount -= int(amount)/len(myapps)
                            final_objs.save()
            return HttpResponseRedirect(reverse('myapp:room_detail',kwargs={'pk':pk}))
        except:
            messages.error(request, "Details do not match the specified data type. (Hint: amount should be integer)")
            return HttpResponseRedirect(reverse('myapp:room_detail',kwargs={'pk':pk}))
    # return render(request,'myapp/create_transaction.html',{'members':members,'rooms':rooms})
...
Update all the final_transactions objects between previous transaction myapps and user to delete all the debts created by that transaction.
Again update final_transactions objects according to the new data i.e. between new myapps and user with new amount(depending on the data given).
...
def update_transaction(request,pk,id):
    ...
    prev_myapps = prev_transaction.myapps.all()
    prev_myapps_usernames = [x.username for x in prev_myapps]
    if request.method == "POST":
        new_reason = request.POST.get('reason')
        new_amount = request.POST.get('amount')
        new_transaction_members = request.POST.getlist('myapp')
        new_myapps = User.objects.filter(username__in = new_transaction_members)
        obj = transaction(room = rooms,amount=new_amount,reason=new_reason,payer=request.user)
        obj.save()
        obj.myapps.add(*new_myapps)
        for x in prev_myapps:
            if x == request.user:
                pass
            else:
                ...
                if len(if_user_receiver_prev) == 1 and len(if_user_sender_prev) == 0:
                    final_objs = final_transactions.objects.get(sender=x,receiver=request.user)
                    final_objs.final_amount -= int(prev_amount)/len(prev_myapps)
                    final_objs.save()
                else:
                    final_objs = final_transactions.objects.get(sender=request.user,receiver=x)
                    final_objs.final_amount += int(prev_amount)/len(prev_myapps)
        ...
        for x in new_myapps:
            if x == request.user:
                ...
                debt_obj = debt(room = rooms,transaction=obj,sender=x,receiver=request.user,amount=int(new_amount)/len(new_myapps))
                debt_obj.save()
                if_user_sender = final_transactions.objects.filter(sender=request.user,receiver=x)
                if_user_receiver = final_transactions.objects.filter(sender=x,receiver=request.user)
                if len(if_user_receiver) == 0 and len(if_user_sender) == 0:
                    final_obj = final_transactions(sender=x,receiver=request.user,final_amount=int(new_amount)/len(new_myapps))
                    final_obj.save()
                else:
                    if len(if_user_receiver) == 1 and len(if_user_sender) == 0:
                        final_objs = final_transactions.objects.get(sender=x,receiver=request.user)
                        final_objs.final_amount += int(new_amount)/len(new_myapps)
                        final_objs.save()
                    else:
                        final_objs = final_transactions.objects.get(sender=request.user,receiver=x)
                        final_objs.final_amount -= int(new_amount)/len(new_myapps)
...
def my_debts(request):
    ...
    return render(request,'../templates/my_debts.html',{'incomes':income,'expenses':expense})
...
def final_settlements(request):
    ...
    if len(user_sender_positive) == 0 and len(user_receiver_negative) == 0:
        noexpense = True
    return render(request,'../templates/final_settlements.html',{'user_sender_positive':user_sender_positive,
...
def delete_debt(request,pk):
    if request.method == "POST":
        ...
        if len(if_user_receiver) == 1 and len(if_user_sender) == 0:
            ...
            final_objs.save()
        obj.delete()
        return HttpResponseRedirect(reverse('myapp:my_debts'))
    return HttpResponseRedirect(reverse('myapp:my_debts'))
...
def personal_budget(request):
    ...
    template = '../templates/personal_budget.html'
    return render(request, template, context)
...
def add_personal_budget(request):
    if request.method == 'POST':
        ...
        try:
            ...
            form.save()
            return HttpResponseRedirect(reverse('myapp:personal_budget'))
        except:
            messages.error(request, "Details do not match the specified data type. (Hint: amount should be integer)")
            return HttpResponseRedirect(reverse('myapp:personal_budget'))
...
def delete_personal_income(request,pk):
    if request.method == "POST":
        ...
        obj.delete()
        return HttpResponseRedirect(reverse('myapp:personal_budget'))
    return render(request,'../templates/personal_budget.html',{'obj':obj})
...
def delete_personal_expense(request,pk):
    if request.method == "POST":
        ...
        object.delete()
        return HttpResponseRedirect(reverse('myapp:personal_budget'))
    return render(request,'../templates/personal_budget.html',{'object':object})
